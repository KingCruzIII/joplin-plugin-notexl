...
function validateScreenshots(screenshots) {
  ...
  for (const screenshot of screenshots) {
    ...
    if (!allPossibleScreenshotsType.includes(screenshotType))
      throw new Error(`${screenshotType} is not a valid screenshot type. Valid types are: \n${allPossibleScreenshotsType}\n`);
...
function createPluginArchive(sourceDir, destPath) {
  const distFiles = glob.sync(`${sourceDir}/**/*`, { nodir: true, windowsPathsNoEscape: true }).map((f) => f.substr(sourceDir.length + 1));
